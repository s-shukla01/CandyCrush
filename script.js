...
function candyCrushGame() {
  ...
  function dragLeave() {
    // No clearing background here to avoid flicker
  }
  ...
  // Utility: Animate and clear matched candies
  function animateAndClear(indices, points) {
    score += points;
    scoreDisplay.innerHTML = score;
    // Add matched animation class
    indices.forEach(index => squares[index].classList.add('matched'));
    // After animation, clear candies and remove class
    setTimeout(() => {
  ...
  function checkColumnForFour() {
    for (let i = 0; i < 39; i++) {
      ...
      if (
        ...
        animateAndClear(columnOfFour, 4);
      }
    }
  }
  ...
  function checkColumnForThree() {
    for (let i = 0; i < 47; i++) {
      ...
      if (
        ...
        animateAndClear(columnOfThree, 3);
      }
    }
  }

  // Main game loop with slightly longer interval to reduce lag
  window.setInterval(() => {
    checkRowForFour();
    checkColumnForFour();
    checkRowForThree();
